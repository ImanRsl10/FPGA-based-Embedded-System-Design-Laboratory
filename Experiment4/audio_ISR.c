...
//extern volatile unsigned int el_buf[BUF_SIZE];					// audio buffer
//extern volatile unsigned int er_buf[BUF_SIZE];					// audio buffer
extern volatile int KEY_value;
extern volatile int record_flag;
extern volatile unsigned int temp_buf[BUF_SIZE];
...
void audio_ISR(struct alt_up_dev *up_dev, unsigned int id)
{
	...
	if (alt_up_audio_read_interrupt_pending(up_dev->audio_dev))	// check for read interrupt
	{
		...
		if (buf_index_record < BUF_SIZE)
		{
			...
			(void) alt_up_audio_record_l (up_dev->audio_dev, &(l_buf[buf_index_record]), num_read);
			buf_index_record += num_read;

			if (buf_index_record == BUF_SIZE)
			{
				// done recording
				alt_up_parallel_port_write_data (up_dev->green_LEDs_dev, 0); // turn off LEDG[5]
	...
	if (alt_up_audio_write_interrupt_pending(up_dev->audio_dev))	// check for write interrupt
	{
		...
		else if (buf_index_play < BUF_SIZE/2 && KEY_value == 0x8) //DownSample
		{
			alt_up_parallel_port_write_data (up_dev->green_LEDs_dev, 0x80); // set LEDG[7] on

			num_written = alt_up_audio_play_r (up_dev->audio_dev, &(temp_buf[buf_index_play]),
					BUF_SIZE/2 - buf_index_play);
					/* assume that we can write the same # words to the left and right */
			(void) alt_up_audio_play_l (up_dev->audio_dev, &(temp_buf[buf_index_play]),
				num_written);
			buf_index_play += num_written;

			if (buf_index_play == BUF_SIZE/2)
			{
				// done playback
				alt_up_parallel_port_write_data (up_dev->green_LEDs_dev, 0); // turn off LEDG[7]
				alt_up_audio_disable_write_interrupt(up_dev->audio_dev);
				KEY_value = 0x0;
			}
		}
